...
impl GateClient {
    ...
    pub async fn fetch_futures_accounts(&self, settle: &str) -> Result<Value> {
        let path = format!("/api/v4/futures/{}/accounts", settle);
        self.signed_request(Method::GET, &path, "", "")
            .await
            .with_context(|| format!("failed to GET futures accounts for {}", settle))
    }
    ...
    pub async fn fetch_user_trades(
        ...
        let mut query = format!("contract={}", contract);
        ...
        let path = format!("/api/v4/futures/{}/my_trades", settle);
        self.signed_request(Method::GET, &path, &query, "")
            .await
            .with_context(|| format!("failed to GET user trades for {}", contract))
    }
